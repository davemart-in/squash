  ...
  try {
    ...
    if (startStep <= 5) {
      ...
      const fixPrompt = [
        ...
        `3. Implement the fix.`,
        `4. Commit your changes with a descriptive commit message. Use --no-verify to skip hooks.`,
        ``,
    ...
    if (startStep <= 6) {
      ...
      const prPrompt = [
        ...
        ``,
        `1. First, run: git push --no-verify -u origin ${branch}`,
    ...
    if (startStep <= 7) {
      ...
      const reviewPrompt = [
        ...
        `3. If you find problems, fix them, amend the commit (--no-verify), and force-push (--no-verify).`,
